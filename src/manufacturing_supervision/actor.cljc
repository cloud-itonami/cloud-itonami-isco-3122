(ns manufacturing-supervision.actor
  "ManufacturingSupervisionActor — the ISCO-08 3122 independent
  manufacturing-supervision actor as a `langgraph.graph/state-graph`
  (per ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  supervision operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the SupervisionAdvisor can never
  directly commit a record or dispatch a robot action the
  ManufacturingSupervisionGovernor refuses — every commit-record! call
  is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [manufacturing-supervision.advisor :as advisor]
            [manufacturing-supervision.governor :as governor]
            [manufacturing-supervision.store :as store]))

(defn build-graph
  "Build a compiled ManufacturingSupervisionActor graph. `store`
  implements `manufacturing-supervision.store/Store`. `advisor`
  implements `manufacturing-supervision.advisor/Advisor` (defaults to
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:line-id (:line-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
