(ns manufacturing-supervision.advisor
  "SupervisionAdvisor — proposes a manufacturing-supervision operation
  (supervise a shift, inspect equipment, clear a safety-hold on
  production equipment, override a quality-control failure) for a
  registered line. The advisor is swappable: `mock-advisor`
  ...
  of line provenance or safety-hold/quality-override risk;
  `manufacturing-supervision.governor` is the independent system that
  decides whether the proposal may proceed, per the itonami actor
  pattern.

  A proposal is a map:
    {:op :supervise|:inspect|:clear-safety-hold|:override-quality-failure
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for line " (:line-id request))})
...
(def ^:private system-prompt
  "You are a manufacturing-supervision advisor. Given a supervision
